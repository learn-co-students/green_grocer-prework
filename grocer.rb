...
# attribute_hash.each do |attribute_symbol, attribute_value|
  ...
  # cart_hash = cart_hash.uniq
              #     cart_hash = {food_item_key => {attribute_symbol => attribute_value, 
  # attribute_symbol => attribute_value, :count => food_item_frequency}}
    end 
          #  binding.pry # After 3 exits, cart_hash IMproperly returns merely 
              # {"AVOCADO"=>{:price=>3.0, :clearance=>true, :count=>2} . 
              # At this point,  cart_hash  should be returning a hash of 3 key/value pairs (including dupes, depending on the     cart     ).
  end 
  cart_hash
          #  binding.pry   # After 3 exits, cart_hash IMproperly returns merely 
              #{"AVOCADO"=>{:price=>3.0, :clearance=>true, :count=>2} . 
